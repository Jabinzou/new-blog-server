...
export function qiniuUpload(stream = 'file', localFile, key) {
  ...
  const config: LooseObject = new qiniu.conf.Config();
  // 空间对应的机房
  ...
  return new Promise((resolve, reject) => {
    formUploader[stream === 'stream' ? 'putStream' : 'putFile'].call(formUploader,
        uploadToken, key, localFile, putExtra, (respErr, resBody, respInfo) => {
      if (respErr) {
        reject(respErr);
      }
